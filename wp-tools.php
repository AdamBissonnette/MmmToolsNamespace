...
namespace MmmToolsNamespace;
	...
	function createFormField($label, $name, $value, $type, $options=array())
	{
		...
		switch ($type)
		{
			...
			break;
			case 'date':
				$field = createInputWithClass($label, $value, $type, $options, "date");
			break;
			case 'datetime':
				$field = createInputWithClass($label, $value, $type, $options, "datetime");
			break;
			case 'button': case 'link':
				$field = createLink($label, $name, $options, "btn");
				$showLabel = false;
			break;
			case 'text': default:
	...
	function createLink($label, $value, $options = array())
	{
		extract( merge_options(
			array("class" => "",
				"note" => "",
		...
		$linkTemplate = '<a href="%s" id="%s" class="btn %s">%s%s</a>';
		$iconTemplate = "";

		if ($icon != "")
		{
			$iconTemplate = sprintf('<i class="fa %s"></i> ', $icon);
		}

		$output = sprintf($linkTemplate, $href,
			 $label, //id
			 $class,
			 $iconTemplate,
	...
	function createInput($label, $value, $type="text", $options = array())
	{
		extract( merge_options(
			array("class" => "",
				"placeholder" => "",
		...
		$inputTemplate = '<input type="%s" id="%s" class="%s" name="%s" value="%s" placeholder="%s"%s />';

		if ($icon != "")
		{
			$inputTemplate = sprintf('<i class="fa %s"></i> ', $icon) . $inputTemplate;
		}
		...
		$output = sprintf($inputTemplate, $type,
			 $label, //id
			 $class,
			 $name, //name
			 stripslashes($value), //value
	...
	function createInputWithClass($label, $value, $type, $options, $add_class)
	{
		$class = getStringValueFromArray($options, "class");

		$options["class"] = $class . " " . $add_class;

		return createInput($label, $value, $type, $options);
	}
